#!/usr/bin/env python3
"""
Cleanup-Welle map_derived_variables (2026-06-23, post Migration #9).

Entfernt 3 Flags ohne aktive Konsumenten (verifiziert per Sheet- und Live-
Workflow-Grep):
  - needs_detangling
  - styling_goal_natuerlich
  - needs_protection_focus

Bereinigt zusätzlich die `konsumenten`-Annotation bei `heat_use`
(entfernt den Verweis `flags.needs_protection_focus`, der jetzt obsolet ist).

Vorgehen analog migrate_node05_sheet.py: clear() + Header + verbleibende
14 Datenzeilen, Re-Read-Verifikation.

Vorbedingung: Backup in backups/sheets_20260623_pre_node05_cleanup/.
"""
...
# Reihenfolge: Phase 1 (nur normalized.*-Refs) → Phase 2 (mit flags.*-Refs)
ROWS = [
...
REMOVED = ["needs_detangling", "styling_goal_natuerlich", "needs_protection_focus"]
...
def main():
    ...
    grid = build_grid()
    print(f"Plan: {len(grid)-1} Zeilen + Header in Tab '{TAB}' (Vorher: 17, Nachher: {len(grid)-1})")
    print(f"Entfernt: {', '.join(REMOVED)}")
    print(f"Header: {grid[0]}")
    print("Stichproben:")
    for i in (1, 2, 7, 12, 14):
        if i < len(grid):
            r = grid[i]
    ...
    rows_after_nonempty = [r for r in rows_after if any(c.strip() for c in r)]
    print(f"\nWrote {len(grid)} rows ({len(HEADER)} cols). Sheet has {len(rows_after_nonempty)} non-empty rows.")
    if len(rows_after_nonempty) != len(grid):
        print("WARN: row-count mismatch", file=sys.stderr)
        return 1
    for i, (planned, actual) in enumerate(zip(grid, rows_after_nonempty), start=1):
        if planned != actual:
