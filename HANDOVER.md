...
| #27 | **Sync-Skript-POC — 3/10 Sheets in Workflow eingemeißelt** (deployed 2026-07-02) | 3 Nodes: `map_conflict_rules → 14 Konflikte auflösen` (`rules`), `map_priorities → 04 Prioritäten auflösen` (`priorityRules`), `map_slot_rules → 11 REQ-Regeln auswerten` (`ruleItems`) — je embedded JSON zwischen SYNC-Markern | Kern der Skalierungs-Roadmap: `sync_rules_to_workflow.py` (~290 LOC) liest Sheets via gspread, patched Verbraucher-Node-jsCode-Konstanten zwischen `// >>> SYNC:{tab}` / `// <<< SYNC:{tab}`-Markern, PUT + GET-Verifikation, Backup in `backups/workflow_sync/`. Zwei Emit-Stile: `"stmt"` (ersetzt komplette `const X = $items(...).map(...);`-Zeile) und `"expr"` (ersetzt nur `$items(...).map(...)`-Sub-Ausdruck, filter-Chain bleibt intakt). Optionales `post_init_replace` pro SYNC-Entry für einmalige Sekundär-Ersetzungen (z.B. Node 11 hat `.map(it => it.json)` in getrennter `const rules = ruleItems.map(...).filter(...)`-Deklaration nach `ruleItems` — muss beim init einmal weg, sonst würden Plain-Dicts auf `.json` mappen und leere Regeln erzeugen). **Semantik-Check pro Sync-Schritt via Single-Profile Anna**: 0 Slot-Drift + identische `applied_conflict_rules`/`applied_rules` gegen Migration-#21-Baseline 2026-06-28. **Bulk-Ampel nach 3 Syncs (gap=30, 13 Profile)**: 2/13 success (tom+sina = letzte im Trigger-Fenster, 0 Slot-Drift bei tom, sina-Drift `bond_iq_leave_in→bond_iq_night_day_serum` ist Post-Baseline-Effekt aus Migration #21 V1 nicht durch Sync). 11/13 crash weiterhin an Google-Sheets-Quota — Crash-Node-Verteilung: 9× Node 06d Profil-Funktion-Mapping, 1× 06c Max-Products, 1× **Node 04a Prioritäten** (verwaister Loader, wird von Node 04 nicht mehr konsumiert, verbraucht aber weiter Quota → Konsequenz aus POC-Design-Entscheidung „Loader drin lassen"). Nächste 7 Sheets: `map_derived_variables` (Node 05), `map_pflegelevel_scoring/-overrides/-max_products` (Node 06), `map_profil_funktion` (Node 12), `produktdatenbank` (Node 07 — größter Payload, 37×25≈1000 Zellen, PUT-Body-Größe zum ersten Mal potenziell kritisch), `map_pool_filter` (Node 08). Ampel wird auf grün gehen, wenn entweder alle 7 gesynct **oder** die verwaisten Loader (04a, 10, 13) parallel aus dem Workflow entfernt werden — verwaiste Loader werden im nächsten Session-Block adressiert. **Session-2-Nebenprodukt**: HANDOVER Z. 47 (`map_conflict_rules`) korrigiert — real 6 Zeilen statt „CON-01 bis CON-12": CON-02/07/09/11/12/13, CON-02/09/12 seit Migration #12 aktiv=FALSE, CON-01/03/04/05/06/08/10 vor 2026-07-02 gelöscht (Historie nicht mehr rekonstruierbar). Backups: `backups/workflow_sync/workflow_backup_20260702_111946_pre_sync.json` (Schritt 1), `..._150510_...` (Schritt 2 map_priorities), `..._202207_...` (Schritt 2 map_slot_rules). |
...
**Nächste-Session-POC** ~~geplant~~ **abgeschlossen 2026-07-02** (Migration #27): 3/10 Sheets synced (`map_conflict_rules` → Node 14, `map_priorities` → Node 04, `map_slot_rules` → Node 11), Sync-Skript `sync_rules_to_workflow.py` etabliert (~290 LOC, Marker-basiert, idempotent, `stmt`/`expr`-Emit-Stile, optionaler `post_init_replace`). 0 Slot-Drift bei Anna vs. Migration-#21-Baseline. Bulk-Ampel gelb (2/13). Nächste Schritte: (a) restliche 7 Sheets syncen (Reihenfolge: priorities/slot_rules → derived_variables → pflegelevel-trio → profil_funktion → pool_filter → **zuletzt produktdatenbank** wegen ~1000 Zellen Payload); (b) verwaiste Loader (04a, 10, 13) aus dem Workflow entfernen sobald alle Konsumenten gesynct sind — Bulk-Ampel geht dann auf grün.
