...
| Element | Wert |
|---|---|
| Produktname (user-facing) | **mybeautykey** (Codename `myglowmatch` bleibt in Repo/Ordner/Package/Workflow-IDs, siehe Rename-Notiz unten) |
...
|---|---|---|
| `produktdatenbank` | 37 Produkte × 33 Spalten (produkt_key, produktname_de, produktlinie, produkttyp, slot_typ, routine_schritt, kopfhaut, haarstruktur, haarstaerke, haarzustand, hauptfunktion, nebenfunktionen, pflegelevel, ausschluss_bei, ist_hitzeschutz, ist_bonding, ist_scalp_focus, locken_geeignet, kombinationen, kombi_optional, aktiv, produkt_url, anwendung, **intensitaet** (NEU 2026-06-18), **ist_trockenshampoo**, **ist_zwei_in_eins**, **ist_oel**, **ist_peeling**, **ist_smoothing** (alle 5 NEU 2026-07-09 Layer-1-Refactor), **ist_haarwuchs**, **ist_curl_volumen_booster** (beide NEU 2026-07-10 Slot-Override-Refactor), **ist_bonding_line**, **ist_hitzeschutz_solo** (beide NEU 2026-07-12 Layer-2-Runde 1)) | Node 07 (via Sync in Node 08 embedded) |
...
| `map_pflegelevel_scoring` (Schema-Erw. 2× 2026-06-24) | 23 Punktevergabe-Regeln, 10 Spalten (`regel_id \| kategorie \| bedingung_typ \| bedingung_feld \| bedingung_wert \| punkte \| max_punkte \| suppress_if \| beschreibung \| aktiv`). Operatoren: `equals` / `in_list` / `array_contains` (Match → fixe Punkte) und `array_count_except` (count × punkte, gecappt auf `max_punkte`). `suppress_if`-Spalte (Migration #13) ermöglicht Mutex zwischen Regeln (CSV-Liste; PFL-02/03 haben `suppress_if=PFL-01`). PFL-23 (Migration #10) ist erste aggregate-Regel. | Node 06a |
...
| `map_slot_rules` | REQ-Regeln (26 aktive Regeln, Trigger + Filter, seit 2026-07-10 alle markenneutral). Neue Spalte `slot_override_erlaubt` (Layer-1-Refactor 2026-07-10) — TRUE bei REQ-06/16b, sonst FALSE. | Node 10 (via Sync in Node 11 embedded) |
| `map_conflict_rules` | CON-Regeln (6 Zeilen Stand 2026-07-02: CON-02/07/09/11/12/13; CON-02/09/12 seit Migration #12 `aktiv=FALSE` behalten; CON-01/03/04/05/06/08/10 zwischenzeitlich gelöscht, Historie nicht mehr rekonstruierbar), match_typ + action + **slot_scope** (NEU 2026-07-12 Layer-2-Runde 1: kommagetrennte Slot-Liste oder leer) | Node 13 |
| `map_pool_filter` | POOL-01 (Bonding) und POOL-03 (Locken-Styling). POOL-02 (Gewicht) bewusste Lücke. | Node 08a |
| `map_derived_variables` (zuletzt erweitert 2026-06-25) | 16 JSON-Regeln für Bool-/Enum-Flags (`variable \| typ \| regel_json \| erlaubte_werte \| konsumenten \| doku`). Phase 1 R02-R13 nur `normalized.*`-Refs, Phase 2 R14-R17 mit `flags.*`-Refs. Migration #15 fügte `prefers_straight` (Phase 1) und `wants_full_curl_line` (Phase 2) hinzu, modifizierte `needs_curl_care` / `styling_goal_definition` / `curl_refresh_needed`. Sheet-Reihenfolge = Auswertungs-Reihenfolge. | Node 05a → Node 05 |
...
## Workflow-Nodes (27 inkl. Test-Mode-Check)
...
| 6 | **05a map_derived_variables laden** (NEU 2026-06-23) | googleSheets | **`map_derived_variables`** — 16 JSON-Regeln |
...
| 12 | 06 Pflegelevel berechnen | code | **v5 (deployed 2026-06-24, Migration #13)**: 100% sheet-getrieben — kein Inline-Special-Case mehr. Zwei-Phasen-Auswertung: Phase 1 alle Regeln in fired-Map, Phase 2 `suppress_if`-Auflösung (PFL-02/03 via PFL-01), Phase 3 Summe. Operatoren: `equals`/`in_list`/`array_contains`/`array_count_except`. System-Parameter (LOW/MID/HIGH-Schwellen) bleiben im Code. Phase 5+6 via `map_pflegelevel_overrides`, Phase 7 via `map_max_products`. |
...
| #12 | **CON-02/09/12 → `ausschluss_bei` (Architektur-Verlagerung)** (deployed 2026-06-24) | `produktdatenbank.ausschluss_bei` (4 Zellen-Updates) + `map_conflict_rules` (3 Toggles `aktiv=FALSE`) | **Keine Code-Änderungen** — beide Mechaniken (Node 08 `ausschluss_bei`-Match, Node 14 `aktiv=FALSE`-Skip) bereits vorhanden. Logik-Verschiebung von „Konflikt-Resolver entfernt Produkt aus existierendem Pool" zu „Produkt kommt gar nicht erst in den Pool". Edits: `fohncreme.ausschluss_bei=wellig,lockig,kraus` (CON-09), `essig_shampoo`/`essig_spuelung.ausschluss_bei=normal,trocken,juckend_empfindlich` (CON-12 ∪ CON-02), `kopfhaut_peeling.ausschluss_bei=juckend_empfindlich` (CON-02). CON-02/09/12 → `aktiv=FALSE` (für Audit-Trail behalten, nicht gelöscht). **0/36 Routing-Drift** (Executions vs. Baseline 522-528, Final-Routine byte-identisch). `applied_conflict_rules` schrumpft sichtbar: Maria CON-09/11/12 → CON-11; Lena CON-09/11 → CON-11; Julia CON-07/12 → CON-07; Bianca CON-02/09 → []; Vivien CON-09/11/12 → CON-11; Sarah CON-09/11 → CON-11. Architektonisch sauberer: „Produkt X unpassend für Profil Y" gehört in Produkt-Stammdaten, nicht in Konflikt-Regeln. Voraussetzung war K-10-Audit (HANDOVER Z. 595): hatte die 6 Edits als Redundanz erkannt, Architektur-Frage als separater Task. Migration ist semantik-erhaltend für die 7 Test-Profile (alle `scalp_status` Single-Element); für hypothetische Multi-Element-`scalp_status`-Profile leicht strenger, weil `ausschluss_bei` über alle Array-Elemente matched während CON `primary_scalp_state` (priorisiert) nutzt. Backups: `backups/sheets_20260624_pre_con_to_ausschluss/produktdatenbank.csv` und `map_conflict_rules.csv`. |
| #13 | **Node 06 PFL-Mutex Sheet-getrieben** (deployed 2026-06-24) | `map_pflegelevel_scoring` (Schema-Erweiterung 9→10 Spalten: neue Spalte `suppress_if`, 2 Cell-Edits PFL-02/PFL-03) | **Node 06 v5** (deployed 2026-06-24): Inline-`hasStarkGesch && ['PFL-02','PFL-03'].includes(...)`-Block (4 LOC) durch zwei-Phasen-Auswertung ersetzt. Phase 1: alle Regeln auswerten in fired-Map. Phase 2: Regeln mit `suppress_if`-ID in fired-Map verwerfen. Phase 3: Summe + firedRules. `suppress_if` ist CSV-Liste (analog `requires_not` in Node 11), bei 21 Regeln leer, bei PFL-02/PFL-03 = `PFL-01`. **Damit ist Node 06 100% sheet-getrieben** — kein einziger Special-Case mehr inline. **0/36 Routing-Drift** (Executions 543-549 vs. Baseline 536-542), `pflegelevel_raw` byte-identisch (Anna 0, Maria 7, Lena 15, Julia 4, Bianca 7, Vivien 4, Sarah 18). **Mutex-Sanity-Check Sarah**: `fired_rules` enthält PFL-01 ohne PFL-02/PFL-03 — Mutex über Sheet-Regel funktioniert wie der ehemalige Inline-Block. Skripte: `migrate_node06_phase3.py`, `patch_node06_phase3.py`. Backups: `backups/sheets_20260624_pre_node06_phase3/map_pflegelevel_scoring.csv`, `workflow_backup_20260624_pre_node06_phase3.json`. |
| #14 | **CON-13: Smoothing-Föhn-Spray bei Locken+Frizz blockieren** (deployed 2026-06-24) | `map_conflict_rules` (neue Zeile CON-13) + `map_slot_rules` (REQ-12 optional→required_conditional, overrides=REQ-13) | Sina-Test offenbarte: bei `hair_structure=kraus + hair_condition=frizz` wählte Scoring smoothing_fohn_spray (glättet aktiv) für styling_1, zudem fielen alle Curl-Slots durch max_products-Cap raus. CON-13: `needs_curl_care=TRUE` + `hair_condition contains frizz` → smoothing_fohn_spray excluden (curl_creme behandelt Frizz bei Locken besser). REQ-12 (curl_creme\|gelee styling_2): optional→required_conditional + overrides=REQ-13 → Curl-Produkt verlässt nicht mehr den Cap. 0/36 Drift bei 7 Test-Profilen (Lena+Sarah hatten bereits hitzeschutzspray statt smoothing, kein Frizz-Match bei anderen). Sinas Routine bekam endlich `curl_gelee` als styling_2 + `hitzeschutzspray` statt smoothing. Backups: `backups/sheets_20260625_pre_curl_fix/`. |
| #16 | **Hitze-Frage präzisiert + Smoothing-Hitzeschutz für glatt+Frizz** (deployed 2026-06-25) | `map_derived_variables` (16→18 Zeilen) + `map_slot_rules` (REQ-04 + REQ-04b NEU + REQ-12) + Frontend `src/data/questions.ts` (heat_frequency-Title + heat_tools-Frage gelöscht) | PDF-belegt (MONAT-Smoothing-PDFs sagen „Hitzeschäden durch Glätteisen und Lockenstäbe", Föhn nicht erwähnt). Frontend-Frage 6b (heat_tools) war Dead Data — Backend liest nur heat_frequency. Backend-Anpassungen: neue derived variables `needs_curl_gelee_styling` (entkoppelt REQ-12 von heat_use=yes — bei Sina-ähnlichen Profilen mit nie_selten+wants_full_curl_line=true greift curl_gelee weiterhin) und `prefers_straight_with_frizz` (Trigger für REQ-04b). Neue REQ-04b: Locken-Träger:in mit Glätt-Wunsch + Frizz + Glätteisen → smoothing_fohn_spray styling_1 (Hitzeschutz + Frizz-Reduktion PDF-belegt in einem Produkt). REQ-04 weicht durch requires_not. Test-Profile (`test_suite.py`): Föhn aus heat_tools entfernt für alle, heat_frequency entsprechend angepasst — Vivien neu als realistisches Hitze-Profil (regelmaessig+lockenstab statt regelmaessig+fohn). **0/8 Drift** (Set-basiert vs. Baseline 561-577). Skripte: `migrate_node16_curl_routing_v2.py`. Backups: `backups/sheets_20260625_pre_migration16/`. |
| #15 | **Curl-Routing-Refactor** (deployed 2026-06-25) | `map_derived_variables` (14→16 Zeilen) + `produktdatenbank` (1 Zelle) + `map_slot_rules` (REQ-04/11/12/13/20 + REQ-11b neu) | Komplettes Refactoring der Curl-Logik. **Neue derived variables**: `prefers_straight` (Locken-Träger:in wählt 'glatt'), `wants_full_curl_line` (Locken-Träger:in wählt 'beides'). **Geänderte derived variables**: `needs_curl_care` zusätzliches `AND curl_priority != 'glatt'` (deaktiviert Curl-Routing bei 'glatt'-Wahl), `styling_goal_definition` matcht jetzt `in [mehr_definition, beides]`, `curl_refresh_needed` matcht `neq taeglich` (statt nur 1x_pro_woche). **Sheet-Stammdaten**: `curl_auffrischer.slot_typ` von styling_2 auf styling_3 (eigener Slot, kein Konflikt mit curl_gelee). **REQ-Regeln**: REQ-04 `requires_not = REQ-11,REQ-11b` (Hitzeschutz weicht für Curl-Creme bei wants_full_curl_line); REQ-11 Filter auf `curl_creme` (nur, statt curl_creme\|gelee); **REQ-11b NEU** trigger `wants_full_curl_line=TRUE` → curl_creme styling_1 required_conditional (auch bei heat_use=yes); REQ-12 Filter auf `curl_gelee` (Slot-Separation); REQ-13 slot_typ=styling_3, prioritaet=required_conditional, zusätzlicher Trigger `wants_full_curl_line=TRUE` (vermeidet curl_auffrischer-Verdrängung bei normalen Locken-Profilen); REQ-20 `requires_not = REQ-05` (scalp_comfort_serum weicht wenn Behandlung feuert — kein Doppel-Scalp). **Frontend-Edit** `src/data/questions.ts`: curl_priority bekommt 4. Option `glatt` ("lieber glatt tragen"). **Validation**: 0/36 Drift für 7 Test-Profile (Executions 561-567 vs. Baseline 553-559). Sina als 8. Test-Profil hinzugefügt — Erwartung: curl_creme + curl_gelee + curl_auffrischer in der Routine, kein smoothing_fohn_spray, kein hitzeschutzspray (durch wants_full_curl_line-Override), kein scalp_comfort_serum. Anomalie-Checks in `test_suite.py` erweitert: Locken-Profil ohne Curl-Produkt + Smoothing-Föhn bei Locken+Frizz. Skript: `migrate_node15_curl_routing.py`. Backups: `backups/sheets_20260625_pre_migration15/`. |
| #17 | **needs_dry_shampoo PDF-Bug-Fix** (deployed 2026-06-26) | `map_derived_variables` (2 Zellen: regel_json + doku auf `needs_dry_shampoo`) | the_champ-PDF („Öl-Absorber für zwischen Wäschen, Frisuren am zweiten Tag") wurde durch invertierten Trigger verraten: vorher feuerte `needs_dry_shampoo` bei `wash_frequency='taeglich'` — semantisch falsch (täglich-Wäscher brauchen keinen Öl-Absorber). Korrekt: `wash_frequency != 'taeglich' AND scalp_status contains 'fettig'`. Eva als 10. Test-Profil (fettige Kopfhaut + alle_2_3_tage + glatt) hinzugefügt — bekommt the_champ als Optional. Drift: Vivien (täglich) verliert the_champ (gewollt). Slot-Drift-Tests (0/36) hatten den Bug nicht abgefangen, weil alle Profile mit demselben Bug liefen — Motivation für späteres PDF-vs-Routing-Audit (→ #21). Keine Workflow-/REQ-Änderung. |
| #18 | **heat_frequency 4→3 Optionen + heat_use 3-stufig** (deployed 2026-06-26) | Frontend `src/data/questions.ts` (heat_frequency 4→3, `sehr_haeufig` gestrichen) + `map_derived_variables` (`heat_use` enum yes/maybe/no statt bool) + `map_pflegelevel_scoring` (PFL-15b NEU, 1 Pkt bei `gelegentlich`) + `map_slot_rules` (REQ-04/REQ-04b `trigger_wert2` von `yes` auf `yes|maybe`) + `map_derived_variables` (`needs_curl_gelee_styling` heat_use `eq yes` → `neq no`) | Punkt 2 aus Dead-Question-Audit: `heat_frequency` hatte 4 Optionen mit nur 2 Backend-Pfaden. `nie_selten==gelegentlich` und `regelmaessig==sehr_haeufig` waren funktional identisch. Refactor: 3 distinct Optionen (`nie_selten`, `gelegentlich`, `regelmaessig`), Backend mit Mittelstufen-Pfad (`heat_use=maybe`). Test-Profile: Lena/Sarah `sehr_haeufig`→`regelmaessig` (keine Drift), Julia `nie_selten`→`gelegentlich` (gewollte Drift: +hitzeschutzspray). 8/10 unverändert. |
| #19 | **wash_frequency 3→2 Optionen konsolidiert** (deployed 2026-06-26) | Frontend `src/data/questions.ts` (3→2 Optionen: `taeglich`, `nicht_taeglich`) | Punkt 3 aus Dead-Question-Audit: `alle_2_3_tage` und `1x_pro_woche` waren funktional identisch (Backend matched nur `neq 'taeglich'`, keine MONAT-PDF differenziert „Tag 3" vs „Tag 7"). Reines String-Mapping, 0 Backend-Edits (`needs_dry_shampoo` + `curl_refresh_needed` matchen weiter `neq 'taeglich'`). 8 Test-Profile umgestellt. **0/10 Drift**. Bewusst NICHT umgesetzt: Wunsch-Frage „möchtest du seltener waschen" — keine MONAT-Linie ist PDF-belegt als Wäsche-Reduktions-Trainings-Pflege. |
| #20 | **mehr_volumen-Routing + Node-12-Engine-Patch** (deployed 2026-06-27) | `map_derived_variables` (`wants_curl_volume` NEU) + `map_slot_rules` (REQ-16b NEU: trigger `wants_curl_volume=TRUE`, slot_typ=`styling_3`, filter=`moxie_mousse`, `required_conditional`) + Workflow Node 12 (Engine-Patch Z. 171) | Punkt 1 aus Dead-Question-Audit: `curl_priority='mehr_volumen'` war routing-tot. PDF-Recherche bestätigte moxie_mousse als idealen Match (Header „Volumen und Dichte / Locken & Wellen", FAQ Q2 „Locken oder Wellen für definierte Form und Frizz-Kontrolle"). **Engine-Patch nötig**: `moxie_mousse.slot_typ=styling_1` (Stammdaten) kollidierte mit REQ-16b.slot_typ=styling_3 — vor Patch fielen Produkte mit Slot-Typ-Mismatch aus `candidates`, bevor der `produkt_key`-Filter griff. Patch ergänzt: bei `produkt_key`-Literal-Filter (kein `=`) werden Match-Produkte als zusätzliche candidates hinzugefügt, REQ-Regel-slot_typ ist authoritative. **Bonus-Bugfix**: REQ-06 (IR Clinical Serum bei `duenn`) hatte denselben Slot-Typ-Mismatch und wurde ignoriert — Maria bekommt jetzt korrekt ir_clinical_kopfhautserum für dünn werdendes Haar (vorher kopfhaut_peeling). Nina als 11. Test-Profil (lockig+fein+kraftlos+mehr_volumen) → curl_creme styling_1 + moxie_mousse styling_3. Drift: 9/11 unverändert, Nina + Maria gewollt. Damit Dead-Question-Audit abgeschlossen (#18+#19+#20). |
| #21 | **PDF-Audit-Korrekturen V1/V2/V5/V6** (deployed 2026-06-28) | `map_derived_variables` (2 Zellen: `needs_repair_focus`, `detangling_need`) + `map_slot_rules` (REQ-04.requires_not + 2 append: REQ-11c, REQ-19b) + `test_suite.py` (Tina+Tom als V5-/V6-Coverage-Profile) | Ergebnis des dreiphasigen Routing-Regel-vs-PDF-Audits (Inventar `audit_phase1_inventar.md` → Plausibilitäts-Check `audit_phase2_verdacht.md` → PDF-Befunde `audit_phase3_befunde.md`). **V1 (🔴)**: `needs_repair_focus` löst jetzt auch bei `gefaerbt` (vorher nur `blondiert`/`stark_geschaedigt`). PDF-belegt in 3/4 Bond-IQ-Datenblättern (WARUM/Bullets nennen „färbst"/„coloriertem Haar"/„Färben"/„chemische Behandlungen" explizit). **V2 (🟡)**: `detangling_need.leicht_verfilzt` matcht jetzt `kraus` zusätzlich zu `lockig`. PDF-belegt via entwirrungsspray-Header-Untertitel „Alle Haarstrukturen" (K-08). **V5 (🔴)**: REQ-11c neu — bei `needs_curl_care + heat_use=maybe` → curl_creme styling_1; REQ-04.requires_not um REQ-11c erweitert. PDF-belegt: curl_creme-Anwendung „mit Diffuser föhnen oder stylen". **V6 (🔴)**: REQ-19b neu — Volumen-Styling jetzt auch bei `hair_thickness=mittel` (parallel zur Original-REQ-19 für `fein`). PDF-belegt: moxie_mousse FAQ „feinem bis mittlerem Haar"; volumen_spray Header-Untertitel „Feines bis mittleres Haar" (K-08). **Coverage-Profile**: Tina (wellig+fein+gelegentlich, V5-Test) und Tom (glatt+mittel+volumen, V6-Test) zu `test_suite.py` (11→13 Profile). V4 (`hair_condition=duenn`/REQ-06) als 🟢 verifiziert: `questions.ts` Z.125 hat den Wert, Maria deckt ihn bereits ab. **Drift-Validation**: 7/13 Profile mit erwartetem Drift, 6/13 byte-identisch. **0 Anomalien** im Post-Lauf. V1 wirkt bei bianca/lena/maria/silvia/tina/vivien (alle `gefaerbt`); V5 bei tina (einzige `heat_use=maybe`+Curl-Struktur); V6 bei tom (einzige `mittel+volumen+nicht-Locken`); V2 ohne direkte Wirkung in den Test-Profilen (Lena/Sina trafen bereits über andere Pfade auf Entwirrungsspray). Bemerkenswert: `wants_intense_care`-Asymmetrie zu `needs_repair_focus` (V7) durch V1 implizit aufgelöst. Drift-Report: `audit_migration21_drift.md`. Backups: `backups/sheets_20260628_224212_pre_migration21/`. Skripte: `audit_phase1_inventar.py`, `migrate_21_pdf_audit.py`, `diff_test_results.py`. Zusätzlich: REQ-14.reason in der Phase-2-Vorbereitung auf Migration-#17-Stand synchronisiert (Doku-Drift). |
| #23 | **Entwirrungsspray-Trigger PDF-präzise** (deployed 2026-07-01) | `map_derived_variables.detangling_need.regel_json` (2 Zellen: regel_json + doku) | Sinas Live-Profil vom 30.06. bekam Entwirrungsspray zusätzlich zur Curl-Creme, obwohl im Fragebogen weder „verfilzt" noch „verheddert" existiert. Ursache: `detangling_need = leicht_verfilzt` feuerte auf zwei überinterpretierten Pfaden — (a) `hair_condition contains trocken UND NICHT haarbruch`, (b) `hair_structure ∈ {lockig, kraus} UND NICHT keine_probleme`. Damit bekam **jede Lockenträgerin mit irgendeinem Problem** Entwirrungsspray, obwohl das PDF explizit „Frizz kontrollieren" als IDEAL-Bullet nennt und nicht Locken-Struktur allein. **Neu**: `stark_verfilzt` bei `haarbruch|spliss` (Header-Untertitel), `leicht_verfilzt` bei `frizz` (IDEAL-Bullet), sonst `problemlos`. Drift-Prognose (Regel ist trivial, Live-Test nicht nötig): Bianca/Nina/Tina verlieren Entwirrungsspray (nur schwache Alt-Trigger, kein PDF-Beleg), Sina bleibt drin — aber als `leicht_verfilzt` (frizz-Pfad) statt `stark_verfilzt` (kraus+deutlich_trocken). Backup: `backups/sheets_20260701_pre_entwirrung_fix/`. |
| #24 | **P1+P2 aus Phase-5-Trigger-Audit** (deployed 2026-07-01) | `map_derived_variables.styling_goal_halt.regel_json` (P1) + `map_slot_rules.REQ-03` (P2: trigger_flag/trigger_wert/reason) | Ergebnis Phase-5-Audit: alle 22 REQ-Regeln mit produkt_key-Filter gegen jeweiliges MONAT-PDF geprüft (drei parallele Explore-Agenten, Konventionen K-06/07/08). Konsolidierung `audit_phase5_trigger_befunde.md`: 15 🟢 sauber, 6 🟡 präzisierbar, 2 Design-Choices, 0 🔴 Widerspruch. **P1 (REQ-16 moxie_mousse Struktur-Filter raus)**: `styling_goal_halt` verlangte bisher `hair_structure ∈ {glatt, wellig}` — PDF sagt aber „Alle Haarstrukturen / Frizz / Volumen und Dichte / Locken & Wellen" im Header + FAQ „feinem bis mittlerem Haar" ohne Struktur-Einschränkung. Locken-Trägerinnen mit aufwendigem Styling wurden zu Unrecht ausgeschlossen. Neu: nur `styling_effort=aufwendiges_styling AND hair_thickness ∈ {fein, mittel}`. **P2 (REQ-03 super_feuchtigkeitsmaske)**: Trigger war `needs_repair_focus=TRUE AND pflegelevel>=3` — hieß intern „Reparaturmaske", PDF-IDEAL nennt aber „Trockenes Haar regenerieren", kein Repair-Bezug. Neu: `hair_condition_contains=trocken AND pflegelevel>=3`. Live-Verifikation via test_suite.py (Mail-Versand über Node 27 blockiert): 10/13 OK, 3 Fehler transient Sheets (siehe #25). Kritisch: **Vivien** (nicht-trocken, gefärbt, MID) verliert REQ-03-Trigger, **erhält Maske aber weiter via REQ-10** („Pflegemaske optional bei MID+") — kein Kundinnen-Verlust. Sarah bekommt zusätzlich moxie_mousse als Optional im styling_1 (durch P1) neben dem required hitzeschutzspray. Backup: `backups/sheets_20260701_pre_migration24/`. P3–P5 aus Audit (REQ-11c doku-refresh, REQ-13 wash_frequency, REQ-08/17b konsolidieren) sind niedrigere Priorität, kein Kundinnen-Schaden. |
| #25 | **Retry auf allen googleSheets-Loadern** (deployed 2026-07-01) | 11 googleSheets-Nodes im Live-Workflow (Nodes 04a, 05a, 06a, 06b, 06c, 06d, 07, 08a, 10, 13, 19) — je `retryOnFail=true`, `maxTries=3`, `waitBetweenTries=5000ms` | Motivation: der Bulk-Test-Run von #24 zeigte 3/13 Fehler durch transient Google-Sheets-„Service unavailable" (Google-API-Quota-Hit bei ~143 Loader-Calls in 15 Min). HANDOVER-Konvention „bei transient einfach nochmal triggern" war Live-Blindspot — bei echten Kundinnen kein manueller Retrigger möglich, Workflow endet still, keine Mail, keine Alerts. Ohne Retry rechnerisch ~1 stille Fail/Tag bei 100+ Anfragen → 1 Support-Anfrage/Tag. Mit Retry: praktisch null Fails. Skript `patch_retry_sheets_loaders.py` mit settings-Cleanup-Fix (n8n API 400 „request/body/settings must NOT have additional properties" bei `availableInMCP`/`timeSavedMode`/`binaryMode`/`callerPolicy` — nur `executionOrder` + wenige weitere sind erlaubt). Backup: `workflow_backup_20260701_pre_retry.json`, lokale Kopie `workflow_retry_v1.json`. Verifikation via GET nach PUT bestätigt retry=True auf allen 11 Loadern. **Neue Ops-Konvention** (in Memory als [[feedback-myglowmatch-produktions-robustheit]]): Jeder externe API-Call im Live-Workflow braucht dieses Retry-Default. **Folge-Punkt für nächste Session**: Cache-Layer (Preloader-Workflow empfohlen) vor Skalierung auf 100+ Anfragen/Tag, plus Error-Trigger-Workflow für Alert-Mail bei Fehler nach 3 Retries. |
| #26 | **Cache-Layer TTL 6h — deployed und rolled back am 2026-07-01 (verworfen)** | 4 neue Nodes (Cache prüfen, IF Cache gültig, Cache schreiben, Cache Merge) + 7 Verbraucher-Node-jsCode-Patches (Preamble + `_fromCache`-Helfer) | **Design**: `workflowStaticData('global').cache.tabs`-Snapshot der 10 Loader, TTL 6h, refresh-on-miss. Fallback pro Verbraucher-Node auf `$items(loaderNode)` als Sicherheitsnetz. Purge-Endpoint zunächst mitgeplant, dann verworfen (bei 1×/Monat Sheet-Edit unnötig). **Root Cause des Fails**: Live-Test 13 Profile mit gap=30s ergab 0/13 success, 13/13 error mit Google-Sheets-`429 RESOURCE_EXHAUSTED` an unterschiedlichen Loadern. `Cache schreiben`-Node feuert erst NACH allen 10 Loadern → bei parallelen Cold-Starts sehen alle Executions den Cache leer → 5-13 parallel bombardieren Google-Sheets gleichzeitig → 429-Wave → Cache-Write wird nie erreicht → nächste Anfrage wieder Cache-leer (Kreislauf). Das Race-Problem tritt bei genau dem Szenario auf, für das der Cache gedacht war (Bursts/Skalierung). **Rollback via PUT von `workflow_backup_20260701_pre_migration26.json` sauber, alle 4 Cache-Nodes weg, Live-Workflow zurück auf 27-Node-Zustand.** Learning: n8n-Static-Data-Cache mit Cache-Write-am-Ende ist bei paralleler Ausführung fundamental race-anfällig, weder ein POC-Wert für Skalierung. Skript `patch_migration26_cache.py` + lokale Kopie `workflow_migration26_v1.json` bleiben als „geplant, verworfen" im Archiv. **Konsequenz für nächste Session (siehe Skalierungs-Roadmap unten)**: statt Cache-Design wird die Architektur umgestellt — Sheets werden per Sync-Skript in den Workflow einkompiliert, Live-Workflow macht dann keine Sheets-Reads mehr (außer Node 19 Log-Write). |
| #27 | **Sync-Skript — 10/10 Sheets in Workflow eingemeißelt + Loader-Cleanup** (deployed 2026-07-02 Schritte 1–3, 2026-07-03 Schritte 4–6 vormittags, 2026-07-03 Schritte 7–10 + Cleanup nachmittags) | 6 Sheet→Node-Mappings: `map_conflict_rules → 14` (`rules`, stmt), `map_priorities → 04` (`priorityRules`, expr), `map_slot_rules → 11` (`ruleItems`, expr + `post_init_replace`), `map_pflegelevel_overrides → 06` (`overrideRules`, expr), `map_max_products → 06` (`maxRules`, expr), `map_pool_filter → 08` (`poolRules`, expr) — je embedded JSON zwischen `// >>> SYNC:{tab}` / `// <<< SYNC:{tab}`-Markern | Kern der Skalierungs-Roadmap: `sync_rules_to_workflow.py` (~305 LOC) liest Sheets via gspread, patched Verbraucher-Node-jsCode-Konstanten zwischen SYNC-Markern, PUT + GET-Verifikation, Backup in `backups/workflow_sync/`. Zwei Emit-Stile: `"stmt"` (ersetzt komplette `const X = $items(...).map(...);`-Zeile) und `"expr"` (ersetzt nur `$items(...).map(...)`-Sub-Ausdruck, `.filter/.sort`-Chain bleibt intakt). Optionales `post_init_replace` pro SYNC-Entry für einmalige Sekundär-Ersetzungen (z.B. Node 11 hat `.map(it => it.json)` in getrennter `const rules = ruleItems.map(...).filter(...)`-Deklaration nach `ruleItems` — muss beim init einmal weg, sonst würden Plain-Dicts auf `.json` mappen und leere Regeln erzeugen). **Semantik-Check pro Sync-Schritt via Single-Profile Anna**: 6/6 exec success, 0 Slot-Drift + identische `applied_conflict_rules`/`applied_rules`/`primary_scalp_state`/`max`-Werte gegen Migration-#21-Baseline 2026-06-28. **Bulk-Ampel nach 3 Syncs (2026-07-02)**: 2/13 success (tom+sina, Positionen 12+13 im Trigger-Fenster). **Bulk-Ampel nach 6 Syncs (2026-07-03)**: 2/13 success (silvia+tina, Positionen 10+11) — Successful-Positionen wandern nach vorne, aber Ampel-Farbe unverändert. Silvia byte-identisch zu Baseline; Tina-Drift `entwirrungsspray → bond_iq_leave_in` ist Post-Baseline-Effekt aus Migration #23 (`detangling_need` PDF-präzise) + #21 V1, nicht durch Sync. Crash-Node-Verteilung Post-Schritt-6: 9× Node 06d Profil-Funktion-Mapping (aktiver Loader, Hauptbottleneck), 1× 04a Prioritäten + 1× 06c Max-Products (**beide verwaist!**) → verwaiste Loader-Nodes laufen weiter und verbrauchen Quota, obwohl ihr Output nicht mehr konsumiert wird. **Konsequenz**: Bulk-Ampel-Kipp auf grün braucht Loader-Cleanup (04a, 06b, 06c, 08a, 10, 13 rausnehmen aus Workflow, Verkabelung anpassen), nicht nur weitere Syncs — Sync alleine kippt Ampel nicht linear grün, wie prognostiziert. Verbleibende 4 Sheets: `map_derived_variables` (Node 05), `map_pflegelevel_scoring` (Node 06 letzter Loader-Aufruf 06a), `map_profil_funktion` (Node 12), `produktdatenbank` (Node 07/08, größter Payload 37×25≈1000 Zellen). **Session-2-Nebenprodukt (2026-07-02)**: HANDOVER Z. 47 korrigiert — `map_conflict_rules` real 6 Zeilen (CON-02/07/09/11/12/13), CON-01/03/04/05/06/08/10 vor 2026-07-02 gelöscht. **Session-3-Fund (2026-07-03)**: `$items(loaderNode)`-Row-Count in success-Executions ist NICHT gleich Sheet-Zeilen-Count (Beispiel Anna Exec 759: Sheet map_pflegelevel_overrides 4 rows vs. `$items("06b …")` 96 rows = Faktor 24). Vermutung: n8n Multi-Item-Input-Semantik + Retry-Multiplikation. Empty-Row-Filter-Divergenz-Check via `$items()`-Count damit unbrauchbar; Sheet-Wahrheit ist die einzige verlässliche Row-Count-Quelle. Follow-up für Sync v2. Backups lokal in `backups/workflow_sync/` (gitignored). **Fortsetzung 2026-07-03 nachmittags**: Schritte 7-10 deployed (map_derived_variables → Node 05 mit 19 Regeln, map_pflegelevel_scoring → Node 06 mit 24 Regeln, map_profil_funktion → Node 12 mit 9 Mappings, produktdatenbank → Node 08 mit 37 Produkten × 25 Spalten, Node-08-jsCode wächst auf 40 KB, Workflow-JSON insgesamt 192 KB). Alle Single-Anna-Tests (Executions 791/795/796) byte-identisch zur Baseline. **Loader-Cleanup deployed** via `cleanup_loader_nodes.py`: Chain war strikt linear (jeder Loader hat exakt 1 Pred + 1 Succ), Cleanup mechanisch (pred → loader → succ wird zu pred → succ, connections[loader] weg, loader aus nodes). Live-Workflow: 27 → 17 Nodes, verbleibende Chain `02 → 04 → 05 → 06 → 08 → 09 → 11 → 14 → 12 → 15 → 17 → 18/18b → 19`. Node 19 (Log-Write) einziger Sheets-Call. **Bulk-Ampel kippt auf grün**: 13/13 success bei gap=30s, alle Executions durchgelaufen innerhalb 15s nach dem letzten Trigger. Anna-Poll-Zahl von ~285 auf 7 gefallen (Latenz-Sprung ~40×). Skripte: `sync_rules_to_workflow.py` (10 SYNCS-Einträge, ~350 LOC), `cleanup_loader_nodes.py` (~145 LOC). Backups: `backups/workflow_sync/workflow_backup_20260703_224021_pre_loader_cleanup.json`. **Sheet-Edit-Konvention aktiv** (K-17): jede Änderung an einem der 10 gesyncten Sheets erfordert `python3 sync_rules_to_workflow.py`, sonst nicht live sichtbar. Doku-Drift bei nächster Sitzung: HANDOVER Z. 36-50 „Vom aktuellen Workflow genutzte Tabs" formuliert Live-Read, ist nach Cleanup irreführend. Zwei obsolete Kommentar-Zeilen in Node 06/08 verweisen noch auf entfernte Loader (semantisch harmlos). |
| #28 | **Layer-1-Wirkungs-Refactor — Engine-Präzisierung + 23 markenneutrale REQ-Regeln** (deployed 2026-07-09/10) | `produktdatenbank` (5 neue Bool-Spalten: `ist_trockenshampoo`, `ist_zwei_in_eins`, `ist_oel`, `ist_peeling`, `ist_smoothing`) + `map_slot_rules` (23 REQ-Regel-Filter markenneutral migriert) + Workflow Node 12 (Wirkungs-Filter-Block auf exakten Token-Match umgestellt) | **Node 12 Wirkungs-Filter präzisiert** (`patch_engine_filter_precision.py`): naive `.includes()`-Substring-Semantik ersetzt durch `p.produkt_key === fStr \|\| p.produktlinie === fStr \|\| hfSet.has(fStr) \|\| nfSet.has(fStr)` (mit `csvToArr` für HF/NF). Bool-Flag-Syntax `ist_X=TRUE` unverändert. Migration-#20-Substring-Block (Slot-Override) noch unverändert — kommt in #29 weg. **10 Cluster in Folge migriert**: Cluster 1 Bond-IQ (REQ-02/21/22 → `ist_bonding=TRUE`), Cluster 2 Scalp-Comfort (REQ-05/20 → `ausgleichend`), Cluster 3a Trockenshampoo+2-in-1 (REQ-14 → `ist_trockenshampoo=TRUE`, REQ-30 → `ist_zwei_in_eins=TRUE`), Cluster 4 Öl-Familie (REQ-08/17/17b → `ist_oel=TRUE`), Cluster 5 Volumen (REQ-19/19b → `volumen`), Cluster 5b Halt (REQ-16 → `halt` — 1 akzeptierter Drift: sarah `moxie_mousse → volumen_spray`, fachlich vertretbar), Cluster 6 Kopfhaut-Peeling (REQ-24 → `ist_peeling=TRUE`), Cluster 7 Restore (REQ-23 → `feuchtigkeit`), Cluster 8 Smoothing/Föhn (REQ-04b → `ist_smoothing=TRUE`), Cluster 3b Curl-Perfection (REQ-11/11b/11c/12/13 → `locken`), Cluster 10 Entwirrung (REQ-07/18 → `entwirren`). **Neue Konventionen**: (a) Vor-Simulation vor jeder Engine-Semantik-Änderung (pro-Filter-Simulation ALT vs. NEU gegen aktuelle Produktdatenbank, um latent tote/lebendige Regeln zu finden — Vermeidung der Vormittags-Falle); (b) Bool-Flag für Kategorie/Technologie (bonding, hitzeschutz, trockenshampoo, 2-in-1) vs. Wirkungs-Token für funktionale Wirkung am Haar (ausgleichend, feuchtigkeit, reparatur); (c) Cluster-Bulk-Pflicht: 0-Drift-Bulk nach jedem Cluster gegen die vorherige Cluster-Baseline. **Bulk-Kette 10× 0-Drift + 1 akzeptierter Drift**. Baseline am Ende: `test_results_20260710_125920.json`. Vormittags-Fehlschlag am 2026-07-09 (naive `.includes()`-Semantik im Additional-Candidates-Block hatte unbeabsichtigte Kandidaten gezogen) war die Motivation für die Präzisierung. Backups: `backups/sheets_20260709_*` + `backups/sheets_20260710_*` (10 Snapshots pro Cluster). Skripte: `patch_engine_filter_precision.py`. Session-Doku: `chat-archive/2026-07-09_abend_engine_praezisierung_und_cluster_1_2.md`. |
| #30 | **Layer-2 Runde 1 — 4 Regeln markenneutral + Node-14-Engine erweitert** (deployed 2026-07-12) | `produktdatenbank` (2 neue Bool-Spalten: `ist_bonding_line`, `ist_hitzeschutz_solo`, Grid 31 → 33) + `map_pool_filter` (POOL-01 auf `ist_bonding_line:is_true`) + `map_conflict_rules` (neue Spalte `slot_scope`; CON-07 auf `bool_flag=ist_hitzeschutz_solo`; CON-11 auf `bool_flag=ist_smoothing` + `slot_scope=shampoo,spuelung,styling_1`; CON-13 auf `bool_flag=ist_smoothing` + `slot_scope=styling_1`) + Workflow Node 14 (Engine-Erweiterung 2×) | **Node 14 Engine 2× erweitert** — (a) `patch_node14_bool_flag.py`: neuer `case 'bool_flag'` in `productMatches` (`werte.some(w => String(prod[w]).toUpperCase() === 'TRUE')`) + optionaler `slotScope`-Parameter (single value). (b) `patch_node14_slot_scope_list.py`: `slotScope` splittet auf Komma, prüft `_slots.includes(prod.slot_typ)`. Beide backward-kompatibel — existierende produkt_key/produktlinie/key_contains-Regeln unverändert. **Vier Regeln markenneutralisiert** (`patch_pool01_bonding_line.py`, `patch_con13_bool_flag.py`, `patch_con07_con11_bool_flag.py`): POOL-01 (Bond-IQ-Linie via `ist_bonding_line`), CON-07 (dediziertes Hitzeschutz-Produkt via neues `ist_hitzeschutz_solo`), CON-11 (Smoothing bei Definitions-Ziel, `slot_scope`-Liste schließt `smoothing_tiefenbehandlung` in `maske` aus), CON-13 (Smoothing-Föhn-Spray in styling_1). **Bulk-Kette 3× 0-Drift** (14/14 pro Bulk): `_233558` nach POOL-01, `_003453` nach CON-13, `_105325` nach CON-07/11. Baseline am Ende: `test_results_20260712_105325.json`. **Wichtige Design-Entscheidungen**: (a) `ist_bonding_line` statt Weg-2 mit `hauptfunktion=bonding` — K-06 hat 3 der 4 Bond-IQ-Produkte aus `hauptfunktion` entfernt (kein eigener Bullet, nur Inhaltsstoff-Mechanik), Fallback-Flag war unumgänglich; (b) `ist_hitzeschutz_solo` als semantische Kategorie (nicht Aus-Trick), weil `heat_use=no` per Migration-#16-Fragebogen nur „kein Glätteisen/Lockenstab" bedeutet (Föhn ausgeklammert) und eine Erweiterung auf `ist_hitzeschutz` bei Föhn-Nutzerinnen fohncreme + smoothing_fohn_spray fälschlich rauswerfen würde. **Neue Konventionen**: (a) K-Node-14-Bool-Flag — Konfliktregeln können markenneutrale Bool-Flags + Slot-Constraint nutzen; (b) Layer-2-Prinzip 5 — Bool-Flag muss echte funktionale Kategorie beschreiben, kein 1:1-Rebrand eines produkt_key (K-06-Verstoß in neuer Verpackung); (c) Layer-2-Prinzip 6 — Fragebogen-Semantik ist load-bearing, vor Regel-Umbau exakte `questions.ts`/`map_derived_variables`-Semantik der genutzten Flags klären. **Nebenbefunde für Backlog**: (i) drei REQ-Regeln in `map_slot_rules` mit `ist_bonding=TRUE` sind K-06-Linien-Proxies (Task #6, separate Session); (ii) Fragebogen-Anwendungs-Frage „nutzt du einen Föhn?" fehlt (Layer-3-Kandidat für sauberen Föhn-Produkt-Match); (iii) `demo/bibliothek.html` braucht drei neue Felder (ist_bonding_line, ist_hitzeschutz_solo, slot_scope). Skripte: `patch_node14_bool_flag.py`, `patch_node14_slot_scope_list.py`, `patch_pool01_bonding_line.py`, `patch_con13_bool_flag.py`, `patch_con07_con11_bool_flag.py`. Backups: `backups/sheets_20260711_231116_pool01_pre/`, `backups/sheets_20260712_000700_con13_pre/`, `backups/sheets_20260712_102746_con07_con11_pre/`, 6× `backups/workflow_sync/workflow_backup_20260711_*/20260712_*`. Session-Doku: `chat-archive/2026-07-12_session.md`. |
| #29 | **Slot-Override-Refactor + REQ-03 Reaktivierung — 26/26 REQ-Regeln markenneutral** (deployed 2026-07-10) | `produktdatenbank` (2 neue Bool-Spalten: `ist_haarwuchs`, `ist_curl_volumen_booster`, Grid 29 → 31) + `map_slot_rules` (REQ-03 reaktiviert + Filter `feuchtigkeit`, REQ-06 + REQ-16b `slot_override_erlaubt=TRUE` und Filter markenneutral) + Workflow Nodes 11 + 12 | **Node 11 v3**: `entry`-Objekt in Slot-Assignment um `slot_override_erlaubt: String(r.slot_override_erlaubt \|\| '').toUpperCase() === 'TRUE'` erweitert (durchreichen aus Sheet). **Node 12 Slot-Override-Block ersetzt** (`patch_slot_override_v2.py`): Migration-#20-Substring-Block (Z. 240-251, `!filterStr.includes('=')` + `filterKeys.includes(p.produkt_key)`) **ersatzlos entfernt**. Neuer Block gate-bedingt an `slotDef.slot_override_erlaubt === true`, Match-Semantik identisch zum Wirkungs-Filter unten (exakter Token-Match auf `produkt_key`/`produktlinie` + `csvToArr(...).has(...)` für HF/NF + Bool-Flag-Syntax). **Damit ist Node 12 komplett frei von Substring-Fallen.** **REQ-03 reaktiviert** (`aktiv=TRUE`, `filter=feuchtigkeit`): Cluster 11 Monat-Black-Schluss, matched `replenish_maske` + `super_feuchtigkeitsmaske`, Slot-Vorfilter `maske` reicht. **REQ-06 markenneutral** (`filter=ist_haarwuchs=TRUE`, `slot_override_erlaubt=TRUE`): erster Anlauf mit Wirkungs-Token `haarwuchs` scheiterte an NF-Match bei `ir_clinical_shampoo` (Vor-Simulation im 2026-07-09-Archiv hatte nur HF geprüft), Rollback via Bool-Flag statt via CSV-Restore. **REQ-16b markenneutral** (`filter=ist_curl_volumen_booster=TRUE`, `slot_override_erlaubt=TRUE`): Wirkungs-Token `volumen` matched 5 Produkte, Ranking-Zufall (analog sarah-Drift bei Cluster 5b) hätte moxie_mousse verlieren lassen — Bool-Flag ist der whitelabel-präzise Ausweg (jede Marke setzt das Flag bei ihrem Curl-Volumen-Booster). **Bulk-Kette 5× 0-Drift** (1 verworfener Zwischenschritt) — `_171055` (REQ-03), `_175843` (slot_override_erlaubt-Flag ohne Engine-Support), `_185804` (Engine-Patch), `_221031` (ist_haarwuchs), `_225149` (ist_curl_volumen_booster). Nach Migration keine markenspezifischen Produkt-Keys mehr in Filtern. **Neue Konventionen**: (a) Vor-Simulation muss HF UND NF prüfen (symmetrische Engine-Semantik → symmetrische Analyse); (b) Bool-Flag als whitelabel-präziser Ausweg bei Wirkungs-Token-Rauschen; (c) `ws.add_cols(1)` vor `update_cell` bei neuer Spalte (sonst `APIError: Range exceeds grid limits`). Backups: `backups/sheets_20260710_162423_pre_req03_reactivate/`, `backups/sheets_20260710_173717_pre_slot_override_flag/`, `backups/workflow_backup_20260710_183528_pre_slot_override_v2.json`, `backups/sheets_20260710_205528_pre_req06_markenneutral/`, `backups/sheets_20260710_214617_pre_ist_haarwuchs/`, `backups/sheets_20260710_221624_pre_ist_curl_volumen_booster/`. Skripte: `patch_slot_override_v2.py`. Session-Doku: `chat-archive/2026-07-10_session.md`. Aktuelle Baseline: `test_results_20260710_225149.json`. |

## Skalierungs-Roadmap (2026-07-02, offen)

**Ziel:** 1000+ parallele Anfragen für Abo-Modell tragen können, ohne Support-Explosion. Aktueller Zustand nach Rollback #26: robust für ~100/Tag (Migration #25 fängt transiente Fehler ab), aber Google-Sheets-Quota (60 reads/min/user) ist Bottleneck bei Bursts.

**Design-Entscheidung (2026-07-01 mit Tomi festgehalten):**
- **Sheets bleiben Wahrheit** (unverändertes Edit-Interface für Tomi/Desirée), aber
- **Live-Workflow liest keine Sheets mehr zur Laufzeit**. Stattdessen: ein Sync-Skript `sync_rules_to_workflow.py` liest die 10 Sheets, generiert JSON-Konstanten, patched die 7 Verbraucher-Nodes im Workflow via n8n-API. Nach Sheet-Edit einmal Skript aufrufen → Workflow kennt neue Werte.
- **Nur Node 19 (Log-Write)** bleibt Sheets-Call — Write-Quota separat, kein Read-Bottleneck.

**Vorteil vs. Cache-Ansätze**: Skaliert unbegrenzt (Millisekunden-Reads aus jsCode-Konstanten), keine externe Dependency (kein Redis/DB), keine Race-Conditions, kein neuer Bedienweg.
...
| Prio | Aufgabe | Stelle |
|---|---|---|
| 🟡 (Phase-2-Roadmap, wartet auf Sinas Go) | **Partnerbereich + Self-Service-Onboarding bauen.** Heutiges System hat `partner_id` hartcodiert in Node 17 (`partners`-Objekt mit Eintrag `desiree`). Phase-2-Bausteine (von Desirée 2026-06-30 spezifiziert): (1) Login-Bereich auf `myglowmatch.de` mit Auth-Provider (Clerk/Supabase/Better-Auth — Entscheidung offen); (2) Bestellformular mit AGB- + AVV-Checkbox + Stripe Checkout, per Webhook automatische Partner-Anlage; (3) Welcome-Mail mit URL/QR + Zugangsdaten; (4) Partner-Dashboard mit Self-Service-Feldern (Foto, Mail, Telefon, ggf. Vorstellungstext) → Node 17 liest Partner-Stammdaten aus DB statt aus hartcodiertem Objekt; (5) Lead-Routing pro Partner-ID — Kunden- + Partner-Lead-Mails landen in der jeweiligen Partner-Mailbox; (6) AVV-Mechanik via Checkbox + PDF-Anhang in Bestellbestätigung + Abruf unter `/avv` (Strato-Methode, Art. 28 Abs. 9 DSGVO erlaubt Textform — Klick + Zeitstempel + IP als Beweis speichern); (7) AGB + AVV-Anwaltsprüfung vor Go-Live empfohlen (~300-500 €). **Preismodell** in Diskussion (finale Entscheidung nach Sinas Feedback): monatlich + jährlich mit ~17 % Rabatt (Jahres-Abo = 10 Monate, „2 Monate gratis"). 49 €/39 € netto verworfen wegen 19 % MwSt (Kleinunternehmerinnen-Partnerinnen ziehen keine Vorsteuer). **Zwei Varianten zur Auswahl:** (A) 25 € netto/Monat (29,75 € brutto, „unter 30 €") + 250 € netto/Jahr (297,50 € brutto, „unter 300 €") — Substanz-Signal, ~5 Subscriberinnen Break-Even; (B) 21 € netto/Monat (24,99 € brutto, „unter 25 €") + 210 € netto/Jahr (249,90 € brutto) — niedrigschwelliger, breitere Conversion. Untergrenze 19 € netto (darunter kippt Wertwahrnehmung). **Reihenfolge-Empfehlung:** zuerst Partner-Stammdaten-DB + dynamisches Lead-Routing (Node 17), dann Auth + Dashboard, dann Stripe, parallel Anwalts-Check. Marketing-Material für Sina ausdrücklich auf später verschoben. | `src/`, n8n Node 17, neue DB-Schicht, Stripe, AVV/AGB-Dokumente |
| 🔴 (Pre-Launch-Blocker, sobald Werbe-Versand) | **Double-Opt-In (DOI) für werbliche Mails noch nicht eingebaut.** Aktueller Stand sauber: Formular trennt `consent_recommendation` (Pflicht, transaktionale Empfehlungs-Mail) und `consent_marketing` (separat, Werbe-Einwilligung) — die Routine-Mail selbst braucht **kein DOI**, weil sie auf konkrete Anfrage erfolgt (Vertragserfüllung Art. 6 Abs. 1 lit. b DSGVO). Sobald jedoch Adressen mit `consent_marketing=true` in eine Newsletter-/Werbe-Liste übernommen werden (z. B. via Brevo), greift § 7 Abs. 2 UWG + BGH-Standard → DOI per Bestätigungslink ist Pflicht. **Pre-Launch-Checks vor erstem Werbe-Versand:** (1) Brevo-Liste auf „Confirmed Opt-In" stellen; (2) Bestätigungsmail-Template mit Doppel-Klick-Mechanik aufsetzen; (3) Zeitstempel + IP der Bestätigung speichern (Beweislast); (4) Datenschutzerklärung um Werbe-Zweck + Brevo als Auftragsverarbeiter ergänzen. Reine Empfehlungs-Mails (heutiger Stand) sind ohne DOI rechtlich sauber. | Datenschutzerklärung + Brevo-Listenkonfiguration + ggf. neuer n8n-Sub-Flow für DOI-Bestätigungsmail |
| 🔴 (Pre-Launch-Blocker) | **DKIM-Setup für `veradex.de` bei Strato fehlt.** SPF korrekt (`v=spf1 include:_spf.strato.com -all`), DMARC scharf (`p=reject`), DKIM-Selektor nicht auffindbar (kein `default._domainkey`, kein `strato._domainkey` etc.). Folge: Mails an externe iCloud/Gmail-Empfänger sind im Spam-Risiko (Strato akzeptiert intern mit `250 OK queued`, externe Authentifizierung kann fehlschlagen). Bei Sinas Live-Test 2026-06-29 war unklar, ob die Routine-Mail im Posteingang oder Spam landete. **Vor Go-Live mit echten Endkundinnen oder MyMONAT-Pitch zwingend fixen:** im Strato-Kundenportal DKIM aktivieren, generierten Selektor + Public-Key als TXT-Record setzen, danach via `dig +short TXT <selektor>._domainkey.veradex.de` verifizieren. Strato-UI-Pfad muss noch gefunden werden (Suche 2026-06-30 erfolglos abgebrochen). | Strato-Kundenportal + DNS-Zone `veradex.de` |
...
| 🟢 (erledigt 2026-06-24) | **CON-02/CON-09/CON-12 → `ausschluss_bei` Architektur-Migration abgeschlossen — siehe Migration #12.** 4 ausschluss_bei-Edits (fohncreme/essig_shampoo/essig_spuelung/kopfhaut_peeling), 3 CON-Toggles aktiv=FALSE. **0/36 Routing-Drift**. Damit liegt die „Produkt X unpassend für Profil Y"-Logik architektonisch korrekt in Produkt-Stammdaten (`ausschluss_bei`-Spalte) statt in Konflikt-Regeln. CON-02/09/12 bleiben mit `aktiv=FALSE` im Sheet als Audit-Trail. Konsequenz für Referenzprofil-Tabelle: Bianca hat jetzt 0 CONs gefeuert, Maria/Lena/Vivien/Sarah nur noch CON-11, Julia nur CON-07. | Migration #12 |
...
| ❌ (bewusst nicht umgesetzt 2026-06-24) | **Sheet-Loader-Parallelisierung verworfen.** Kein messbares Performance-Problem (Pipeline-Latenz 3-5 Min, dominiert von Claude-API in Node 17, nicht von Sheet-Reads). Webhook ist `onReceived`, kein synchrones User-Warten. Mehr parallele Google-Sheets-Calls würden das Rate-Limit-Risiko erhöhen (siehe „Bewusste Lücken"). Mittlerer Refactor-Aufwand, hohes Race-Condition-Risiko, kein messbarer Gewinn. Reaktivieren falls Latenz jemals operatives Problem wird. | „Bewusste Lücken" |
| 🟢 (erledigt 2026-06-25) | **Migration #14 + #15 — Curl-Routing-Refactor.** Sina-Test offenbarte Vokabular-Gap (`curl_priority='beides'`), Pool-Cap-Probleme bei Curl-Slots, Smoothing-Föhn-Bug bei Locken+Frizz, Doppel-Scalp-Pflege. **Migration #14** (CON-13 + REQ-12 required_conditional, Quick-Fix). **Migration #15** (Curl-Routing-Refactor): `prefers_straight` + `wants_full_curl_line` neue derived variables, `curl_priority='glatt'` deaktiviert Curl-Routing, `curl_priority='beides'` aktiviert komplette Curl-Linie (curl_creme styling_1 + curl_gelee styling_2 + curl_auffrischer styling_3), Hitzeschutz weicht via `requires_not`, scalp_comfort_serum weicht bei aktivem scalp_comfort_behandlung. Frontend `questions.ts` curl_priority bekam 4. Option `glatt`. Sina als 8. Test-Profil. **0/36 Drift bei 7 Test-Profilen**, Sinas Routine vollständig sheet-konsistent. | Migration #14 + #15 |
| 🟢 (erledigt 2026-06-27) | **Migration #20 — curl_priority='mehr_volumen' Routing + Engine-Patch Node 12.** Punkt 1 aus Dead-Question-Audit: mehr_volumen-Antwort war routing-tot (keine REQ-Regel triggerte darauf). PDF-Recherche bestätigte moxie_mousse als idealen Match (Header: „Volumen und Dichte / Locken & Wellen"; FAQ Q2: „Locken oder Wellen für definierte Form"). Neue derived variable `wants_curl_volume` = needs_curl_care AND curl_priority='mehr_volumen'. Neue REQ-16b: trigger wants_curl_volume=TRUE, slot_typ=styling_3, filter=moxie_mousse, required_conditional. **Engine-Patch in Node 12** war nötig: moxie_mousse.slot_typ='styling_1' (Stammdaten) kollidiert mit REQ-16b.slot_typ='styling_3'. Vor Patch: candidates wurden erst auf slot_typ gefiltert, dann auf filter — moxie_mousse fiel im slot_typ-Filter raus. Patch ergänzt Z. 171: bei produkt_key-Literal-Filter (kein `=`) werden Match-Produkte als zusätzliche candidates hinzugefügt, egal Stammdaten-slot_typ. Damit ist REQ-Regel-slot_typ authoritative. Bonus-Bugfix: REQ-06 (IR Clinical Serum bei dünn werdendem Haar) hatte vor Patch denselben slot_typ-Mismatch (produkt.slot_typ='kopfhaut_taeglich', REQ-06.slot_typ='kopfhaut') — filter wurde ignoriert, kopfhaut_peeling als Fallback. Maria bekommt jetzt korrekt ir_clinical_kopfhautserum für ihr dünn werdendes Haar statt kopfhaut_peeling (das war für fettige Kopfhaut gedacht). Test-Profil Nina als 11. Profil: lockig + fein + kraftlos + mehr_volumen → bekommt curl_creme styling_1 + moxie_mousse styling_3. 9/11 Profile unverändert, 2 Drift gewollt (Nina: erwartet, Maria: Bug-Fix). | erledigt |
| 🟢 (erledigt 2026-06-27) | **Migration #19 — wash_frequency 3→2 Optionen konsolidiert.** Punkt 3 aus Dead-Question-Audit: `alle_2_3_tage` und `1x_pro_woche` waren funktional identisch (Backend matched nur `neq 'taeglich'`). Keine MONAT-PDF differenziert „Tag 3" vs „Tag 7", keine Beratungs-Logik macht hier einen Unterschied. Frontend (questions.ts): 3 → 2 Optionen (`taeglich`, `nicht_taeglich`). Backend: keine Edits (`needs_dry_shampoo` und `curl_refresh_needed` matchen weiter `neq 'taeglich'` — egal welcher konkrete Wert). Test-Profile: alle 8 Profile mit `alle_2_3_tage`/`1x_pro_woche` auf `nicht_taeglich` umgestellt. **0/10 Drift** (reines String-Mapping). UX-Vereinfachung ohne Routing-Verlust. Bewusst NICHT umgesetzt: WUNSCH-Frage „möchtest du seltener waschen" — keine MONAT-Linie ist PDF-belegt als „Wäsche-Reduktions-Trainings-Pflege". Falls künftig E-Mail-Hinweis-Templating mit Wunsch-Trigger gewünscht: separate Architektur-Frage. | `src/data/questions.ts`, `test_suite.py` |
| 🟢 (erledigt 2026-06-27) | **Migration #18 — heat_frequency 4→3 Optionen, heat_use als Enum.** Frontend (questions.ts): `sehr_haeufig` Option gestrichen (war funktional identisch zu `regelmaessig`). Backend: `heat_use` jetzt 3-stufig (yes/maybe/no) statt boolean. `gelegentlich` → heat_use=maybe (eigener Pfad). PFL-15b NEU: 1 Punkt bei gelegentlich (vorher 0). REQ-04 + REQ-04b trigger_wert erweitert auf `yes|maybe` (Hitzeschutz auch bei gelegentlich Glätteisen/Lockenstab). needs_curl_gelee_styling-Regel: `heat_use eq yes` → `heat_use neq no` (curl_gelee bei Locken+Hitze auch bei maybe). Test-Profile: Lena+Sarah sehr_haeufig → regelmaessig (keine Drift, beide heat_use=yes), Julia nie_selten → gelegentlich (gewollte Drift: +hitzeschutzspray, MID/5 statt MID/4). 8/10 Test-Profile unverändert. Alle 3 heat-Pfade jetzt durch Test-Profile gecovered. | `map_derived_variables`, `map_pflegelevel_scoring`, `map_slot_rules`, `src/data/questions.ts` |
| 🟢 (erledigt 2026-06-26) | **Migration #17 — needs_dry_shampoo PDF-Bug fix.** Aktuelle Regel hatte the_champ bei `wash_frequency='taeglich'` getriggert (semantisch invertiert: Trockenshampoo ist ein **Öl-Absorber** für zwischen den Wäschen). PDF-Beleg the_champ: 4× „Öl/Ölansammlungen/überschüssiges Öl absorbieren", „Frisuren am zweiten Tag". Korrekte Regel: `wash_frequency != 'taeglich' AND scalp_status contains 'fettig'` (Öl-Absorber macht nur Sinn bei fettiger Kopfhaut + nicht-täglich-Wäsche). Drift: Vivien (täglich) verliert the_champ — gewollter Bug-Fix. Andere Test-Profile unverändert (alle haben normal/juckend_empfindlich, kein fettig). Neues 10. Test-Profil **Eva** als Coverage: fettig + alle_2_3_tage + glatt + bestmöglich → bekommt the_champ als Optional-Slot. Sheet-Edit war 1 Zelle (regel_json) + 1 Zelle (doku). Kein Workflow-Patch, keine REQ-Regel-Änderung. | `map_derived_variables.needs_dry_shampoo` |
| 🟢 (erledigt 2026-06-25) | **Test-Mode-Check NEU im Workflow** (Node zwischen 17 und 18/18b). IF-Node prüft `$json.first_name endsWith '-TEST'` — bei Test-Profilen aus `test_suite.py` werden Node 18 (Kunden-Mail) und Node 18b (Partner-Mail) übersprungen. Node 19 (Log) bleibt direkt von Node 17 angesteuert und läuft immer. Verifikation: Bulk-Lauf 9 Profile (Anna bis Sina) — Node 18 + 18b 0× gelaufen, Node 19 9× gelaufen, Test-Mode-Check 9× gelaufen. **Postfach bleibt leer bei allen Test-Suite-Läufen.** Live-User (ohne `-TEST`-Suffix) bekommen Mails wie bisher. Skript: `patch_test_mode_check.py`. Backup: `workflow_backup_20260625_pre_test_mode_check.json`. | erledigt |
| 🟢 (erledigt 2026-06-25) | **Migration #16 — Hitze-Frage präzisiert + Smoothing-Hitzeschutz für glatt+Frizz.** Frontend (questions.ts): heat_frequency-Title auf „Wie häufig nutzt du Glätteisen oder Lockenstab?" geändert, heat_tools-Folge-Frage komplett entfernt (war Dead Data, Backend nutzte nur heat_frequency). PDF-belegt: MONAT-PDFs sagen „Hitzeschäden durch Glätteisen und Lockenstäbe" — Föhn nicht erwähnt. Backend: 2 neue derived variables in `map_derived_variables` — `needs_curl_gelee_styling` = (needs_curl_care + heat_use=yes) OR wants_full_curl_line (entkoppelt REQ-12 von heat_use, damit Sina trotz nie_selten curl_gelee bekommt); `prefers_straight_with_frizz` = prefers_straight + frizz in hair_condition. REQ-04b NEU: Smoothing-Föhn-Spray als Hitzeschutz-Slot styling_1 bei Locken-Träger:in mit Glätt-Wunsch + Frizz + Glätteisen (kombiniert Hitzeschutz + Frizz-Reduktion in einem Produkt). REQ-04 requires_not erweitert um REQ-04b. REQ-12 trigger_flag2 von heat_use=yes auf needs_curl_gelee_styling=TRUE. Test-Profile-Update: Föhn aus heat_tools entfernt für alle Profile, heat_frequency entsprechend angepasst (julia/bianca/sina auf nie_selten, anna von gelegentlich auf nie_selten weil heat_tools=[]; vivien als realistisches Hitze-Profil neu modelliert: regelmaessig+lockenstab statt regelmaessig+fohn). **0/8 Drift** (Set-basiert) vs. Baseline 561-577. | erledigt |
...
| maria  | wellig, fein, duenn, kraftlos, gefaerbt, Hitze nie, ausgewogen | **normal** | MID | 7 | 5 | 5 | CON-11 |
| lena   | kraus, dick, trocken, frizz, gefaerbt, regelmaessig Lockenstab, bestmoeglich | **normal** | HIGH | 15 | 10 | 7 | CON-11 |
| julia  | glatt, fein, kraftlos, unbehandelt, gelegentlich Glaetteisen/Lockenstab, ausgewogen | **normal** | MID | 5 | 5 | 5 | CON-07 |
| bianca | wellig, mittel, trocken, gefaerbt, Hitze gelegentlich, ausgewogen | **juckend_empfindlich** | MID | 7 | 5 | 5 | (keine) |
| vivien | wellig, dick, keine_probleme, gefaerbt, regelmaessig Lockenstab, bestmoeglich | **normal** | MID | 4 | 7 | 7 | CON-11 |
| sarah  | lockig, fein, stark_geschaedigt+spliss+trocken, blondiert, regelmaessig Glaetteisen, bestmoeglich | **normal** | HIGH | 18 | 10 | 7 | CON-11 |
| sina (NEU 2026-06-25) | kraus, dick, trocken+frizz, blondiert, kein Hitzestyling, bestmoeglich, curl_priority=beides | **juckend_empfindlich+schuppig** | HIGH | 14 | 10 | 10 | CON-11 |
| nina (NEU 2026-06-27) | lockig, fein, kraftlos, unbehandelt, kein Hitzestyling, nicht_taeglich, ausgewogen, curl_priority=mehr_volumen | **normal** | MID | 6 | 5 | 5 | (keine) |
| silvia (NEU 2026-06-25) | wellig, mittel, trocken+frizz, gefaerbt, regelmaessig Glaetteisen, ausgewogen, curl_priority=glatt | **normal** | HIGH | 12 | 7 | 6 | (keine) |
| eva (NEU 2026-06-26) | glatt, mittel, keine_probleme, unbehandelt, kein Hitzestyling, bestmoeglich, nicht_taeglich | **fettig** | LOW | 3 | 5 | 4 | CON-07 |
...
- **Sheet-Loader-Parallelisierung bewusst nicht umgesetzt (2026-06-24).** Die 7 Loader (04a/05a/06a/06b/06c/06d/08a) laufen sequenziell, nicht parallel. Begründung: (a) Pipeline-Latenz 3-5 Min dominiert von Claude-API in Node 17 + n8n-Cloud-Cold-Start, Sheet-Reads sind ~10-30s davon — kein Bottleneck. (b) Webhook ist `onReceived`, User wartet asynchron auf E-Mail. (c) 4 Sheet-API-Errors im Test-Suite-Lauf am 2026-06-24 (Cleanup-Sarah, Migration #10 Validation, Migration #12 Validation, Validierung Migration #13) zeigten, dass n8n-Cloud's Google-Sheets-Quota fragil ist — Parallelisierung würde Rate-Limit-Treffer verschlimmern, nicht senken. (d) Mittlerer Refactor-Aufwand (alle Loader-Connections umverkabeln), hohes Race-Condition-Risiko in Consumer-Nodes. Reaktivieren falls Latenz jemals operatives Problem wird.
