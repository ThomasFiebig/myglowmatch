...
| 🟡 | **Frontend Hitze-Frage umformulieren** — User-Vorschlag: „Wie häufig nutzt du Glätteisen oder Lockenstab?" statt generisch „Hitze-Styling". Föhn aus den Hitze-Tools rausnehmen. Heat_tools-Folge-Frage entsprechend anpassen oder entfernen. Backend-Implikation: `heat_use`-Flag-Regel bleibt unverändert (basiert weiter auf `heat_frequency`-Häufigkeit), aber Frequenz bekommt jetzt einen anderen Bezugspunkt (nur noch echtes Hitzestyling). Eine separate Iteration, weil semantischer Bruch für bestehende User-Daten. | `src/data/questions.ts`, `map_derived_variables.heat_use` |
...
| vivien | wellig, dick, keine_probleme, gefaerbt, Hitze regelmaessig, bestmoeglich | **normal** | MID | 4 | 7 | 7 | CON-11 |
