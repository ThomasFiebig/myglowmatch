...
| 🟡 | **K-08-Audit `intensitaet`-Spalte für 29 Default-`alle`-Produkte.** Initial-Belegung 2026-06-18 umfasste nur die 8 Tie-Produkte (feuchtigkeits/renew-Linien, entwirrungsspray/rejuvabeads, kopfhaut_peeling/scalp_comfort_behandlung). Restliche 29 Produkte auf `alle`-Default — `intensitaet`-Spalte ist damit unterauditiert. Stufe 11 in Node 12 ist `alle`-neutral (Tiebreak-only-Effekt), daher kein Routing-Risiko durch die Audit-Lücke. Folge-Session: Header-Untertitel + WARUM/IDEAL-Bullet pro Produkt prüfen, `leicht`/`intensiv`/`alle` setzen. K-08-Belegmuster: „leicht"/„schwerelos"/„ohne zu beschweren"/„täglich" → leicht; „tiefenwirksam"/„Ultra-/intensive"/„revolutionär"/„Treatment" → intensiv; Header ohne klare Tier-Aussage → alle. | Sheet `produktdatenbank.intensitaet` |
