#!/usr/bin/env python3
"""
Slot-Override-Refactor v2 (2026-07-10).

Ziel: sauberer, ausschließlich Flag-basierter Slot-Override.
Der alte Migration-#20-Block (produkt_key-Literal-Substring) wird
ersatzlos aus Node 12 entfernt. Stattdessen greift ein neuer Block
nur, wenn `slotDef.slot_override_erlaubt === true`. Match-Semantik
im Override-Block ist identisch zum Wirkungs-Filter unten
(exakter Token-Match für produkt_key/produktlinie, csvToArr.has für
hauptfunktion/nebenfunktionen, Bool-Flag-Syntax für ist_X=TRUE).

Node 11 (11 REQ-Regeln auswerten):
  - slot_override_erlaubt-Feld im Slot-Assignment-Entry ergänzen.

Node 12 (12 Scoring & Slot-Befüllung):
  - Migration-#20-Block (Zeilen 240-251) durch Flag-basierten Block ersetzen.

Nach dem Patch: PUT + GET-Verifikation.

Aufruf:
    python3 patch_slot_override_v2.py             # PUT
    python3 patch_slot_override_v2.py --dry-run   # nur Snapshot lokal
"""
...
NODE12_NEW = """  // PATCH 2026-07-10 (Slot-Override-Refactor v2):
  // Slot-Typ der REQ-Regel ist authoritative über Stammdaten-Slot-Typ,
  // wenn `slot_override_erlaubt=TRUE` im Sheet gesetzt ist. Kein Legacy-
  // Substring-Fallback mehr — Match-Semantik identisch zum Wirkungs-
  // Filter unten (exakter Token-Match + Bool-Flag-Syntax).
...
def apply_patch(node: dict, old: str, new: str, label: str) -> None:
    ...
    print(f"  OK Node-Patch '{label}' angewendet ({len(old)} -> {len(new)} chars)")
...
def main() -> int:
    ...
    backup_path = BACKUP_DIR / f"workflow_backup_{ts}_pre_slot_override_v2.json"
    ...
    apply_patch(n12, NODE12_OLD, NODE12_NEW, "Node 12 Migration-#20-Block -> Flag-basierter Slot-Override")

    if args.dry_run:
        print("[4/4] Dry-run: kein PUT.")
        local = BACKUP_DIR / f"workflow_slot_override_v2_patched_{ts}.json"
    ...
    ok12_new = 'Slot-Override-Refactor v2' in v12["parameters"]["jsCode"]
    ok12_old_gone = 'Migration #20' not in v12["parameters"]["jsCode"]
    print(f"      Verify Node 11 flag: {'OK' if ok11 else 'FAIL'}")
    print(f"      Verify Node 12 new block: {'OK' if ok12_new else 'FAIL'}")
    print(f"      Verify Node 12 old block removed: {'OK' if ok12_old_gone else 'FAIL'}")
    return 0 if (ok11 and ok12_new and ok12_old_gone) else 1
