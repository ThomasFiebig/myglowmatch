#!/usr/bin/env python3
"""
Patch Node 06 jsCode für Migration #13 (PFL-Mutex ins Sheet, 2026-06-24).

Eingang: workflow_live_now.json (frisch via n8n API)
Ausgang: workflow_node06_v5.json (zum n8n-PUT)

Änderung:
  - Inline `hasStarkGesch && ['PFL-02','PFL-03'].includes(...)` raus
  - Zwei-Phasen-Auswertung in der Regel-Loop:
    Phase 1: Alle Regeln auswerten, fired-Map (regel_id → earned_pts) bauen
    Phase 2: Regeln mit suppress_if-ID in fired-Map verwerfen
    Phase 3: Summe pts + firedRules-Append

Damit ist Node 06 100% sheet-getrieben.
"""
...
OUT_PATH = Path("workflow_node06_v5.json")
NODE06_NAME = "06 Pflegelevel berechnen"

NODE06_V5_JSCODE = r"""// ============================================================
// NODE 06 v5: Pflegelevel berechnen — vollständig sheet-getrieben
// ============================================================
...
// 05 → 06a → 06).
//
// Migration #10 (2026-06-24): Phase 2 (Ziele-Bonus) als PFL-23
// via Operator `array_count_except` mit `max_punkte`-Cap.
// Migration #13 (2026-06-24): PFL-Mutex (PFL-01 vs PFL-02/03)
// via neue Spalte `suppress_if`. Zwei-Phasen-Auswertung
// ersetzt den inline `hasStarkGesch`-else-if.
// ============================================================
...
// CSV-Liste-Parser für suppress_if (kann mehrere IDs enthalten, z.B. "PFL-01,PFL-04")
function csvList(v) {
  return String(v || '').split(',').map(s => s.trim()).filter(Boolean);
}

// ── Phase 1: Alle Regeln auswerten, fired-Map bauen ────────
// fired = { regel_id: { rule, earned } }
const fired = {};
for (const rule of scoringRules) {
  const earned = applyRule(rule);
  if (earned > 0) {
    fired[rule.regel_id] = { rule, earned };
  }
}

// ── Phase 2: suppress_if auflösen ──────────────────────────
// Regel verwerfen wenn eine ihrer suppress_if-IDs gefeuert hat.
const finalFired = {};
for (const id of Object.keys(fired)) {
  const { rule, earned } = fired[id];
  const blockedBy = csvList(rule.suppress_if);
  const isBlocked = blockedBy.some(bid => bid in fired);
  if (!isBlocked) {
    finalFired[id] = { rule, earned };
  }
}

// ── Phase 3: Summe + firedRules ────────────────────────────
let pts = 0;
const firedRules = [];
for (const id of Object.keys(finalFired)) {
  const { rule, earned } = finalFired[id];
  pts += earned;
  firedRules.push({
    regel_id: id,
...
// ── Phase 7: Max Products aus map_max_products ─────────────
...
def main():
    ...
    node06["parameters"]["jsCode"] = NODE06_V5_JSCODE
    new_loc = len(NODE06_V5_JSCODE.splitlines())
